...
pub(crate) enum Constraint {
    ...
    ///
    /// Example: `GET local(k)` generates `Subscriptable(typeof(k))`.
    Subscriptable(Ty, Span),
    ...
    ///
    /// Generated by higher-order Iterable functions (`Iter.map`, `Iterable.filter`,
    ...
    /// Type must be monoidal (supports `++` concatenation).
    ///
    /// Generated by the `++` operator. Monoidal types are `String`, `Array[T]`,
    /// and `Map[K, V]`.
    ///
    /// Example: `a ++ b` generates `Monoid(typeof(a))` and `Monoid(typeof(b))`.
    Monoid(Ty, Span),

    /// Type must support bitwise operations (`&`, `|`, `<<`, `>>`).
    ///
    /// Generated by bitwise operators. BitLike types are `Bool`, `Int`, and `Word`.
    ///
    /// Example: `a & b` generates `BitLike(typeof(a))` and `BitLike(typeof(b))`.
    BitLike(Ty, Span),

    /// Type must support unary negation (`-`).
    ///
    /// Generated by unary `-` operator. Negatable types are `Int` and `Float`.
    /// Note: `Word` is NOT negatable since it is unsigned.
    ///
    /// Example: `-x` generates `Negatable(typeof(x))`.
    Negatable(Ty, Span),

    /// Type must be indexable with a given key type.
    ///
    /// Generated by index access (`e[i]`). Indexable types are:
    /// - `Array[T]`: indexed by `Int`, returns `T`
    /// - `Map[K, V]`: indexed by `K`, returns `Option[V]`
    /// - `String`: indexed by `Int`, returns `Char`
    ///
    /// Example: `arr[i]` generates `Indexable { base: typeof(arr), idx: typeof(i), elem: ?e }`.
    Indexable {
        base: Ty,
        idx: Ty,
        elem: Ty,
    ...
    ///
    /// Generated by `AS` casts and `WRITE` statements. The `from` type must
    /// support conversion to the `to` type.
    ///
    /// Example: `x AS String` generates `Into { from: typeof(x), to: String }`.
    /// Example: `WRITE x` generates `Into { from: typeof(x), to: String }`.
    Into { from: Ty, to: Ty, span: Span },

    /// Type can be fallibly converted to another type.
    ///
    /// Generated by `READ` conversions. The `from` type must support fallible
    /// conversion to the `to` type. Unlike `Into`, this is for fallible conversions
    /// that return `Result[T, String]`.
    ///
    /// Example: `x READ Int` generates `TryInto { from: typeof(x), to: Int }`.
    TryInto { from: Ty, to: Ty, span: Span },
}
...
impl Constraint {
    ...
    pub(crate) fn span(&self) -> Span {
        match self {
            ...
            | Self::Monoid(_, span)
            | Self::BitLike(_, span)
            | Self::Negatable(_, span)
            | Self::Indexable { span, .. }
            | Self::Into { span, .. }
            | Self::TryInto { span, .. } => *span,
        }
    }
}
