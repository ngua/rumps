...
impl SolveCtx<'_> {
        ...
        constraints.iter().enumerate().for_each(|(i, pc)| {
            ...
            match &pc.c {
                ...
                Constraint::Callable {
                    ...
                    self.solve_call(callee, &args, ret, *span);
                }
                Constraint::HasField {
                    base,
                    field,
                    ...
                    self.solve_field(base, *field, field_ty, *span);
                }
    ...
    /// Solve a field access constraint.
    fn solve_field(
        &mut self,
        base: TyId,
        field: StringId,
        ...
        match shape {
            ...
            Ty::Union(_, ref members) => {
                ...
                ms.iter().for_each(|&m| {
                    self.solve_field(m, field, field_ty, span);
                });
            }
